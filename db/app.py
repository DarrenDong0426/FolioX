...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///site.db'                 # Configure database and its location. Here it indicates sqlite as the database and site.db relative to this path via ///
...
def hardcode_projects_database():   
    with app.app_context():                                                 # Descriptor to access Flask obect (the database here)                            
        ...
        eecs370_p4_cache = Projects(name="Intro to Computer Organization (EECS 370): Cache", desc="""Simulates movement between cache, processor, and memory. The cache is set-associative with a write-back and least recently used eviction policy.
                            ...
                            lock=True, wip=False, month_year=date(11, 11, 16), language=["C"], type=["Software"])
        ...
        projects_db.session.commit()
        print("Database created and populated.")

if __name__ == '__main__':
    hardcode_projects_database()
