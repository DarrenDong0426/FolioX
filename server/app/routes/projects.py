...
from sqlalchemy import desc, or_, asc                                              # Import descending func and or operatior from sqlalchemy
...
def get_projects():
    ...
    if (dropdown == "Least Recent"):
      pagination = Projects.query.filter(or_(*words)).order_by(                 # Query rows of projects table from the filtered entries ordered by least recent on top. Paginate in groups of 5
            asc(Projects.month_year)
          ).paginate(page=page, per_page=5, error_out=False)      
    elif (dropdown == "A-Z"):
      pagination = Projects.query.filter(or_(*words)).order_by(                 # Query rows of projects table from the filtered entries ordered by ascending alphabet. Paginate in groups of 5
          asc(Projects.name)
        ).paginate(page=page, per_page=5, error_out=False)  
    elif (dropdown == "Z-A"):
      pagination = Projects.query.filter(or_(*words)).order_by(                 # Query rows of projects table from the filtered entries ordered by descending alphabet. Paginate in groups of 5
          desc(Projects.name)
        ).paginate(page=page, per_page=5, error_out=False)
    else:
      pagination = Projects.query.filter(or_(*words)).order_by(                 # Query rows of projects table from the filtered entries ordered by most recent on top. Paginate in groups of 5
